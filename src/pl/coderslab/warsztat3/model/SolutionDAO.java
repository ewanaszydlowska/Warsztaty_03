...
public class SolutionDAO {
	...
	public void saveToDB(Connection conn, Solution s) throws SQLException {
		if (s.getId() == 0l) {
			...
			String sql = "UPDATE solution SET updated=NOW(), description=?, exercise_id=?, users_id=? WHERE id=?;"; // moze
																													// nowe
																													// preparedstatement?
	...
	public static List<SolutionWithUser> loadAll(Connection conn, int count) throws SQLException {
		List<SolutionWithUser> list = new ArrayList<>();
		String sql = "SELECT solution.id, solution.updated, solution.description, solution.users_id, users.username "
				+ "FROM solution JOIN users on solution.users_id=users.id ORDER BY solution.updated DESC "
				+ "LIMIT ?;";
		PreparedStatement ps = conn.prepareStatement(sql);
		ps.setInt(1, count);
		ResultSet rs = ps.executeQuery();
		while(rs.next()) {
			...
			Date updated = rs.getDate("solution.updated");
			...
			SolutionWithUser sol = new SolutionWithUser(new Solution(solutionId, updated, description, usersId), userName);
			list.add(sol);
		}
		ps.close();
		rs.close();
